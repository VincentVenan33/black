@extends('layouts.app', ['page' => __('Ubah Data Pasien'), 'pageSlug' => 'changepasien'])
        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="title">{{ __('Ubah Data Pasien') }}</h5>
                </div>
                <form method="post" action="{{ route('pages.updatepasien') }}" enctype="multipart/form-data">
                    <div class="card-body">
                        ...
                        <div class="form-group{{ $errors->has('nama') ? ' has-danger' : '' }}">
                            <label>{{ __('Nama') }}</label>
                            <input type="text" name="nama" class="form-control{{ $errors->has('nama') ? ' is-invalid' : '' }}" placeholder="{{ __('Nama') }}" placeholder="{{ __('Nomor Rekam Medis') }}" value="{{ old('nama', $pasien->nama) }}">
